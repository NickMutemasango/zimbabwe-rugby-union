...
export const metadata: Metadata = {
  ...
  description:
    "Explore Zimbabwe Rugby Union's national squads — The Sables, Lady Sables, Cheetahs, Lady Cheetahs, and Junior Sables.",
};
...
function TeamCard({
  ...
  return (
    ...
    >
      {/* Background image – cover, no distortion on any screen size */}
      <div
...
export default function TeamsPage() {
  // Resolve the 5 squads in the canonical display order
  const [sables, ladySables, cheetahs, ladyCheetahs, juniorSables] = [
    TEAMS_LIST.find((t) => t.slug === "mens-xv")!,
    TEAMS_LIST.find((t) => t.slug === "womens-xv")!,
    TEAMS_LIST.find((t) => t.slug === "cheetahs")!,
    TEAMS_LIST.find((t) => t.slug === "lady-cheetahs")!,
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="relative pt-28 pb-14 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-slate-50 to-white overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative text-center">
          ...
          <p className="text-gray-400 text-lg max-w-2xl mx-auto">
            From the senior Sables to the next generation — discover all five
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-5">
          {sables && (
            <TeamCard team={sables} className="h-72 sm:h-80 lg:col-span-2" />
          )}
          {ladySables && (
            <TeamCard team={ladySables} className="h-72 sm:h-80 lg:col-span-1" />
          )}
        </div>

        {/* Row 2: Cheetahs, Lady Cheetahs, Junior Sables (equal thirds) */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          {[cheetahs, ladyCheetahs, juniorSables].filter(Boolean).map((team) => (
            <TeamCard key={team.slug} team={team} className="h-64 sm:h-72" />
          ))}
        </div>
      </div>
    </div>
